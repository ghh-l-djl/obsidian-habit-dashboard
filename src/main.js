  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("delete", (file) => {
        noteManager.handleFileDelete(this, file);
      })
    );

    this.registerEvent(
      this.app.metadataCache.on("changed", (file) => {
        noteManager.handleEventMetadataChanged(this, file);
      })
    );
  }
...
class YoriDashboardView extends ItemView {
  ...
  async onClose() {
    ...
    if (this.plugin._timerInterval) {
      clearInterval(this.plugin._timerInterval);
      this.plugin._timerInterval = null;
    }
  ...
  scheduleRender() {
    ...
    this._renderHandle = requestAnimationFrame(() => {
      ...
      try {
        ...
        console.error("[Habit Dashboard] render failed", err);
      }
    });
  }
  ...
  render() {
    ...
    const layout = host.createDiv({ cls: "yd-layout" });
    timerSection.renderSidebar(layout, ctx);
    const root = layout.createDiv({ cls: "yd-root" });
