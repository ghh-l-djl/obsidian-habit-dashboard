...
const { fitTextarea, ensureFocusInput, createIconButton, setIconSafe } = require("../dom-utils");
...
async function postponeUndoneTomorrow(settings, dateKey, app) {
  ...
  if (!date) return { moved: [], nextDateKey: null };
...
function renderDayEvents(parent, ctx, eventOpts) {
  ...
  if (undoneCount > 0) {
    const postponeBtn = head.createEl("button", {
      cls: "yd-postpone-btn",
    ...
    postponeBtn.onclick = async () => {
      ...
      for (const event of moved) {
        if (event.filePath) {
          try {
            await noteManager.updateEventFrontmatter(ctx.app, event.filePath, { date: nextDateKey });
          } catch (e) {
            new Notice(`同步笔记失败: ${e.message}`);
          }
        }
  ...
  const wireAddBtn = (addBtn) => {
    addBtn.onclick = () => {
      ...
      renderInlineEditor(parent, "", async (value) => {
        ...
        if (!value) return;
        const newEvent = createEvent(settings, dateKey, value);
        try {
          const filePath = await noteManager.createEventNote(ctx.app, { dateKey, title: value });
          updateEvent(settings, dateKey, newEvent.id, { filePath, createdAt: noteManager.formatLocalDateTime(new Date()) });
        } catch (e) {
          new Notice(`创建笔记失败: ${e.message}`);
          deleteEvent(settings, dateKey, newEvent.id);
        }
        await ctx.save();
        ctx.refresh();
      }, () => {
...
function renderEventRow(parent, event, dateKey, ctx, meta) {
  ...
  checkbox.onchange = async () => {
    const completedAt = checkbox.checked ? noteManager.formatLocalDateTime(new Date()) : null;
  ...
  // ── Start-task button (only for uncompleted events) ────────────
  if (!event.completed) {
    const started = !!event.startedAt;
    const startBtn = row.createEl("button", {
      cls: "yd-event-start-btn" + (started ? " yd-event-start-btn--done" : ""),
      attr: {
        type: "button",
        title: started ? event.startedAt : t("calendar.startTask")
      }
    });
    const iconOk = setIconSafe(startBtn, "play");
    if (!iconOk || startBtn.childElementCount === 0) startBtn.setText("▶");

    if (!started) {
      startBtn.addEventListener("click", async (evt) => {
        evt.stopPropagation();
        const startedAt = noteManager.formatLocalDateTime(new Date());
        updateEvent(settings, dateKey, event.id, { startedAt });
        startBtn.addClass("yd-event-start-btn--done");
        startBtn.setAttribute("title", startedAt);
        if (event.filePath) {
          try {
            await noteManager.updateEventFrontmatter(ctx.app, event.filePath, { startedAt });
          } catch (e) {
...
function renderWeeklyView(root, ctx, modal) {
  ...
  for (let i = 0; i < 7; i += 1) {
      ...
      headAdd.onclick = () => {
        renderInlineEditor(col, "", async (value) => {
          ...
          try {
            ...
            updateEvent(ctx.settings, dateKey, mEvt.id, { filePath: mFp, createdAt: noteManager.formatLocalDateTime(new Date()) });
          } catch (e) {
            new Notice(`创建笔记失败: ${e.message}`);
            deleteEvent(ctx.settings, dateKey, mEvt.id);
          }
          await ctx.save();
          renderWeeklyView(root, ctx, modal);
        });
    ...
    if (!isMobileWeek) {
      ...
      addBtn.onclick = () => {
        ...
        renderInlineEditor(col, "", async (value) => {
          ...
          try {
            ...
            updateEvent(ctx.settings, dateKey, evt.id, { filePath: fp, createdAt: noteManager.formatLocalDateTime(new Date()) });
          } catch (e) {
            new Notice(`创建笔记失败: ${e.message}`);
            deleteEvent(ctx.settings, dateKey, evt.id);
          }
          await ctx.save();
          renderWeeklyView(root, ctx, modal);
        }, () => {
      ...
      const undoneForDay = events.filter((e) => !e.completed);
      if (undoneForDay.length > 0) {
        const postponeWeekBtn = col.createEl("button", {
          cls: "yd-week-postpone-btn",
